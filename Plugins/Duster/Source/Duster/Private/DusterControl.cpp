...
FReply UDusterControl::Create2DMaterial()
{
	...
	Divide->ConstB = 10;
	Divide->MaterialExpressionEditorX = -500;
	Divide->MaterialExpressionEditorY = 100;
	...
	Divide->ConnectExpression(&Add->B, 0);
...
void UDusterControl::OnSelectionChanged(const UTypedElementSelectionSet* SelectionSet)
{
	...
	for (TObjectPtr<AActor> Actor : SelectedActors)
	{
		if (const TObjectPtr<UDuster3DComponent> Component3D = Actor->FindComponentByClass<UDuster3DComponent>())
		{
